...
def _optional_import_fitz() -> Any:
    try:
        ...
        raise PdfProcessingError(
            "PyMuPDF (fitz) is required for PDF preprocessing. Install the 'pdf' extra."
        ) from exc
...
def preprocess_pdf(pdf_path: str | Path, output_dir: str | Path, dpi: int = 150) -> PreprocessResult:
    ...
    try:
        for index in range(document.page_count):
            ...
            if plumber_doc is not None:
                try:
                    extracted_tables = plumber_doc.pages[index].extract_tables() or []
                    tables = [[[(cell or "") for cell in row] for row in table] for table in extracted_tables]
                except Exception:
                    tables = []
            ...
            is_scanned = len(text.strip()) < 40 and len(tables) == 0
            pages.append(
                PageArtifact(
                    page_number=index + 1,
                    text=text,
    ...
    return PreprocessResult(pages=pages, rendered_images_dir=images_dir)
