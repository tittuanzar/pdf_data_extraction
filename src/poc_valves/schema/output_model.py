...
def build_output_model(
    ...
    """Build a dynamic Pydantic model from an Excel file.

    Builds and returns a dynamic Pydantic model class based on the
    `Sv2 Field Name` column from the provided Excel file. Also returns
    a mapping of original Sv2 names -> sanitized python attribute names.

    The model fields are annotated as Optional[str] and include
    `description` metadata taken from a description-like column if
    present.
    """
