...
def _value_matches_regex(field: FieldDefinition, value: Any) -> bool:
    if value is None or not field.validation_regex:
        return True
    return re.fullmatch(field.validation_regex, str(value)) is not None
...
def validate_result(
    ...
    page_lookup = {page.page_number: page for page in page_artifacts}

    for field_def in schema.fields:
        extracted = result.fields.get(field_def.field_id)
        if extracted is None:
            status = "missing" if field_def.required else "unverified"
            issues.append(
                ValidationIssue(
        ...
        elif field_def.data_type == "number" and not isinstance(normalized, (int, float)):
            status = "type_error"
            message = "Expected a numeric value"
        elif field_def.data_type == "boolean" and not isinstance(normalized, bool):
            status = "type_error"
            message = "Expected a boolean value"
        elif field_def.data_type == "date" and not isinstance(normalized, str):
            status = "type_error"
        ...
        if extracted.source_page is not None and extracted.source_page in page_lookup:
            page_text = page_lookup[extracted.source_page].text
            if normalized is not None and str(normalized).strip() and str(normalized) not in page_text:
                if status == "ok":
                    status = "unverified"
                    message = "Value not found on cited page text"

        validated_fields[field_def.field_id] = replace(extracted, normalized_value=normalized)
        issues.append(
            ValidationIssue(
                field_id=field_def.field_id,
