...
class OpenAIConfig:
    ...
    def from_env(cls) -> "OpenAIConfig":
        ...
        if not api_key:
            raise RuntimeError(
                "OPENAI_API_KEY is required. Export it in your shell before running."
            )
...
def chunk_pages(page_texts: list[PageText], pages_per_chunk: int) -> list[list[PageText]]:
...
def build_system_prompt() -> str:
    return (
        "You are reconstructing a tabular Field Mapping Register from PDF text. "
        "Return only valid JSON. No markdown fences, no commentary, no preamble. "
        "Each extracted row must be an object matching the requested schema. "
...
def build_user_prompt(chunk_pages: list[PageText]) -> str:
    ...
    return "\n".join(
        [
            "Extract all table rows visible in the following PDF text chunk.",
            "The source is a Field Mapping Register for control valve datasheets.",
            ...
            "",
            "The source table contains 3 tags. If values are pipe-separated, keep them as pipe-separated strings in the output.",
...
def _build_rows_schema() -> dict[str, Any]:
    row_properties: dict[str, Any] = {
        ...
        "similar_field_sv2_field": {"type": ["string", "null"]},
        "similar_field_dependency": {"type": ["string", "null"]},
        "sv2_spec_dependency_terminologies": {"type": ["string", "null"]},
        "sv2_spec_values": {"type": ["string", "null"]},
        "row_number": {"type": ["string", "null"]},
        "field_name": {"type": ["string", "null"]},
        "validations": {"type": ["string", "null"]},
        "notes_and_additional_information": {"type": ["string", "null"]},
    }
    return {
        "name": "field_mapping_register_rows",
        "description": "Structured rows extracted from a field mapping register",
        "strict": True,
        "schema": {
            "type": "object",
...
def _messages_request(config: OpenAIConfig, user_prompt: str) -> dict[str, Any]:
    return {
        "model": config.model,
        "input": [
        ...
        "text": {"format": {"type": "json_schema", "json_schema": _build_rows_schema()}},
    }


def _call_openai(config: OpenAIConfig, user_prompt: str) -> str:
    try:
        from openai import OpenAI
    except Exception as exc:  # pragma: no cover - optional dependency
        raise RuntimeError(
            "openai is required. Install the 'openai' extra or run `uv sync --extra openai`."
        ) from exc

    client = OpenAI(api_key=config.api_key, timeout=config.timeout_seconds)
    response = client.responses.create(**_messages_request(config, user_prompt))
...
def _repair_json(
    ...
    repair_prompt = "\n".join(
        [
            ...
            raw_response,
            "",
            "Return only corrected valid JSON with the same schema.",
        ]
    )
    ...
    for attempt in range(max_parse_retries + 1):
        try:
            ...
            return LlmChunkResult(
                ...
                chunk_label=f"pages {chunk_pages[0].page_number}-{chunk_pages[-1].page_number}",
                warnings=[str(item) for item in parsed.get("warnings", [])] + warnings,
            )
        except Exception as exc:
            warnings.append(f"parse_attempt_{attempt + 1}: {exc}")
            if attempt >= max_parse_retries:
                break
            raw_response = _repair_json(config, raw_response, str(exc))

    raise RuntimeError(
        f"Failed to parse OpenAI response after {max_parse_retries + 1} attempt(s). "
