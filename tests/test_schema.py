...
class SchemaTests(unittest.TestCase):
    def test_grouping_and_indexing(self) -> None:
        schema = SchemaDefinition(
            ...
            fields=[
                FieldDefinition(field_id="a", field_name="A", subcategory="one"),
                FieldDefinition(field_id="b", field_name="B", subcategory="two"),
            ],
            subcategories=["one", "two"],
        )
        self.assertEqual(sorted(group_fields_by_subcategory(schema)), ["one", "two"])
        self.assertEqual(set(build_field_index(schema)), {"a", "b"})
    ...
    def test_extract_sv2_output_uses_parsed_output_schema(self, mock_get_client):
        ...
        mock_response.choices = [MagicMock(message=MagicMock(parsed=ParsedOutput(tags=[])))]
        mock_client.chat.completions.parse.return_value = mock_response
        mock_get_client.return_value = mock_client

        result = extract_sv2_output_from_pages(
            [{"page_number": 1, "text": "sample text", "image_b64": "", "tables": []}],
            guide="guide",
        )

        self.assertIsInstance(result, ParsedOutput)
        self.assertEqual(result.tags, [])
        parse_kwargs = mock_client.chat.completions.parse.call_args.kwargs
        self.assertIs(parse_kwargs["response_format"], ParsedOutput)
