...
class ValidationTests(unittest.TestCase):
    ...
    def test_numeric_and_regex_validation(self) -> None:
        """Test numeric and regex validation."""
        ...
        validated = validate_result(
            ...
            [
                PageArtifact(
                    page_number=1,
                    text="Document No: 123",
                )
            ],
        )
        self.assertEqual(
            validated.fields["doc_no"].normalized_value, 123
        )
        self.assertEqual(
            validated.validation[0].status, "ok"
        )
